...
class StocksInfo:
    def __init__(self, stocks):
        self.stocks = stocks.strip().split(
            ","
        )  # list of stocks whose info needs to be extracted
        ...
        try:
            ...
            for stock in self.stocks:
                ...
                stock_info_list.append(
                    self.to_stock_quote(stock, stock_info.history(period="1d"))
                )

            return stock_info_list
        except Exception as e:
            logger.error("ERROR", f"Unable to get stocks information. Reason: {e}")
            return None
